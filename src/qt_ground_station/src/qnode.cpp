...
bool QNode::init() {
	...
	node = std::make_shared<rclcpp::Node>("qt_ground_station");
	if (!rclcpp::ok()) {
		return false;
	}
        ...
        moveUAV0 = node->create_publisher<qt_ground_station::msg::ControlCommand>("/uav0/px4_command/control_command", 100);
        moveUAV1 = node->create_publisher<qt_ground_station::msg::ControlCommand>("/uav1/px4_command/control_command", 100);
        moveUAV2 = node->create_publisher<qt_ground_station::msg::ControlCommand>("/uav2/px4_command/control_command", 100);
       /*-------------------------------------ros subs-----------------------------------------------*/

        mocapUAV0 = node->create_subscription<qt_ground_station::msg::Mocap>("/mocap/UAV0", 1000, 
                                                          std::bind(&QNode::SubMocapUAV,this,arg::_1,DRONE_UAV0));
        mocapUAV1 = node->create_subscription<qt_ground_station::msg::Mocap>("/mocap/UAV1", 1000, 
                                                          std::bind(&QNode::SubMocapUAV,this,arg::_1,DRONE_UAV1));
        mocapUAV2 = node->create_subscription<qt_ground_station::msg::Mocap>("/mocap/UAV2", 1000, 
                                                          std::bind(&QNode::SubMocapUAV,this,arg::_1,DRONE_UAV2));

        mocapPayload = node->create_subscription<qt_ground_station::Mocap>("/mocap/Payload", 1000, &QNode::SubMocapPayload, this);
 
        UAV0_log_sub = node->create_subscription<qt_ground_station::msg::Topic_for_log>("/uav0/px4_command/topic_for_log", 100, 
                                                                     std::bind(&QNode::SubTopicForLog,this,arg::_1,DRONE_UAV0));
        UAV1_log_sub = node->create_subscription<qt_ground_station::msg::Topic_for_log>("/uav1/px4_command/topic_for_log", 100, 
                                                                     std::bind(&QNode::SubTopicForLog,this,arg::_1,DRONE_UAV1));
        UAV2_log_sub = node->create_subscription<qt_ground_station::msg::Topic_for_log>("/uav2/px4_command/topic_for_log", 100, 
                                                                     std::bind(&QNode::SubTopicForLog,this,arg::_1,DRONE_UAV2));


        UAV0_attitude_target_sub = node->create_subscription<mavros_msgs::msg::AttitudeTarget>("/uav0/mavros/setpoint_raw/target_attitude", 
                                                                            100,
                                                                            std::bind(&QNode::SubThrustSetpointRaw, this, _1, DRONE_UAV0));
        UAV1_attitude_target_sub = node->create_subscription<mavros_msgs::msg::AttitudeTarget>("/uav1/mavros/setpoint_raw/target_attitude", 
                                                                            100,
                                                                            std::bind(&QNode::SubThrustSetpointRaw, this, _1, DRONE_UAV1));
        UAV2_attitude_target_sub = node->create_subscription<mavros_msgs::msg::AttitudeTarget>("/uav2/mavros/setpoint_raw/target_attitude", 
                                                                            100,
                                                                            std::bind(&QNode::SubThrustSetpointRaw, this, _1, DRONE_UAV2));
        /*-------------------------------------ros service-----------------------------------------------*/
        // service to get the parameters from each drone
        serUpdateParameterUAV0 = node->create_service("/uav0/px4_command/parameters", &QNode::loadUAV0para, this);
